...
def test_parsed_refused_completion(
    ...
    open_ai_span = spans[0]
    assert f"{SpanAttributes.GEN_AI_COMPLETION}.0.content" not in open_ai_span.attributes
    assert f"{SpanAttributes.GEN_AI_COMPLETION}.0.refusal" in open_ai_span.attributes
    assert (
        open_ai_span.attributes[f"{SpanAttributes.GEN_AI_COMPLETION}.0.refusal"]
...
def test_parsed_completion_exception(
    ...
    assert span.attributes.get(f"{SpanAttributes.GEN_AI_PROMPT}.0.content") == "Tell me a joke about opentelemetry"
    assert span.attributes.get(f"{SpanAttributes.GEN_AI_PROMPT}.0.role") == "user"
