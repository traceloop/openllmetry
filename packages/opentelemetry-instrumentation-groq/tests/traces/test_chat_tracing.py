...
def test_chat_streaming_with_events_with_content(
    ...
    choice_event = {
        ...
        "finish_reason": "stop",
        "message": {
            "content": 'A joke about OpenTelemetry!\n\nWhy did the OpenTelemetry Span go to therapy?\n\nBecause it was feeling "traced" out and needed to "inject" some self-care into its life!\n\n(Sorry, it\'s a bit of a " instrumentation"-ally-challenged joke, but I hope it "exposes" a smile on your face!)'
        },
    }
    assert_message_in_logs(logs[1], "gen_ai.choice", choice_event)
