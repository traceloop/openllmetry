...
def test_cohere_rerank_legacy(
    ...
    assert (
        cohere_span.attributes.get(SpanAttributes.GEN_AI_REQUEST_MODEL)
        == "rerank-multilingual-v2.0"
    )
    assert (
        cohere_span.attributes.get(
            f"{SpanAttributes.GEN_AI_PROMPT}.{len(documents)}.role"
        )
        == "user"
    )
    assert (
        cohere_span.attributes.get(
            f"{SpanAttributes.GEN_AI_PROMPT}.{len(documents)}.content"
        )
        == query
    )
    ...
    for i, doc in enumerate(documents):
        assert (
            cohere_span.attributes.get(f"{SpanAttributes.GEN_AI_PROMPT}.{i}.role")
            == "system"
        )
        assert (
            cohere_span.attributes.get(f"{SpanAttributes.GEN_AI_PROMPT}.{i}.content")
            == doc
        )

    for idx, result in enumerate(response.results):
        assert (
            cohere_span.attributes.get(f"{SpanAttributes.GEN_AI_COMPLETION}.{idx}.role")
            == "assistant"
        )
        assert (
            cohere_span.attributes.get(
