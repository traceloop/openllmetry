...
spans (see `__init__.py`); it only sets the legacy `gen_ai.system` attribute.
...
#
# Note: `gen_ai.response.finish_reasons` is NOT emitted by this package. The
# finish reason is only recorded per-choice under the legacy, undeclared
# `gen_ai.completion.{n}.finish_reason` attribute (__init__.py:_set_response_
# attributes), not the contract's top-level `gen_ai.response.finish_reasons` —
# a #4362-class gap.
EXPECTED = frozenset(
    {
...
@pytest.mark.vcr
@pytest.mark.default_cassette("test_chat/test_mistralai_chat_legacy")
def test_chat_span_conforms(instrument_legacy, mistralai_client, span_exporter):
    mistralai_client.chat.complete(
        model="mistral-tiny",
        messages=[UserMessage(content="Tell me a joke about OpenTelemetry")],
    )
...
@pytest.mark.vcr
@pytest.mark.default_cassette("test_chat/test_mistralai_streaming_chat_legacy")
def test_streaming_span_conforms(instrument_legacy, mistralai_client, span_exporter):
