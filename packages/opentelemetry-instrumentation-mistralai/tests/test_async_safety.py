...
async def test_apply_prompt_safety_via_to_thread():
    ...
    result = await asyncio.to_thread(
        _apply_prompt_safety, None, kwargs, LLMRequestTypeValues.CHAT, "mistralai.chat"
    )
...
async def test_apply_completion_safety_via_to_thread():
    ...
    await asyncio.to_thread(
        _apply_completion_safety, None, response, LLMRequestTypeValues.CHAT, "mistralai.chat"
    )
