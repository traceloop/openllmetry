...
async def test_apply_prompt_safety_via_to_thread():
    ...
    result = await asyncio.to_thread(
        _apply_prompt_safety, None, args, kwargs, "google_generativeai.generate"
    )
...
async def test_apply_completion_safety_via_to_thread():
    ...
    response = SimpleNamespace(text="reply", candidates=None)
    await asyncio.to_thread(
        _apply_completion_safety, None, response, "google_generativeai.generate"
    )
