...
class SyncChildBoard(FakeBoard):
    def __init__(self, tracer):
        super().__init__()
        self.tracer = tracer

    def create_task(self, task):
        with self.tracer.start_as_current_span("gnap.sync-child") as child:
            assert child.parent is not None
            self.child_parent_id = child.parent.span_id
        return super().create_task(task)


class AsyncChildBoard(FakeBoard):
    def __init__(self, tracer):
        super().__init__()
        self.tracer = tracer

    async def complete_task(self, task_id, result):
        with self.tracer.start_as_current_span("gnap.async-child") as child:
            assert child.parent is not None
            self.child_parent_id = child.parent.span_id
...
def test_gnap_span_is_current_during_sync_operation():
    exporter = InMemorySpanExporter()
    provider = TracerProvider()
    provider.add_span_processor(SimpleSpanProcessor(exporter))
    tracer = provider.get_tracer(__name__)
    instrumentor = GNAPInstrumentor()
    instrumentor._instrument(tracer_provider=provider, board_class=SyncChildBoard)
    try:
        board = SyncChildBoard(tracer)
        board.create_task({"id": "FA-1"})
    finally:
        instrumentor._uninstrument()

    spans = {span.name: span for span in exporter.get_finished_spans()}
    assert board.child_parent_id == spans["gnap.task.create"].context.span_id


@pytest.mark.asyncio
async def test_gnap_span_is_current_during_async_operation():
    exporter = InMemorySpanExporter()
    provider = TracerProvider()
    provider.add_span_processor(SimpleSpanProcessor(exporter))
    tracer = provider.get_tracer(__name__)
    instrumentor = GNAPInstrumentor()
    instrumentor._instrument(tracer_provider=provider, board_class=AsyncChildBoard)
    try:
        board = AsyncChildBoard(tracer)
        await board.complete_task("FA-1", "done")
    finally:
        instrumentor._uninstrument()

    spans = {span.name: span for span in exporter.get_finished_spans()}
    assert board.child_parent_id == spans["gnap.task.complete"].context.span_id
