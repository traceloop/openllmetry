...
async def test_agents_and_tools(instrument_legacy, span_exporter, log_exporter):
    ...
    for tool_span in call_tool_spans:
        assert "tool.name" in tool_span.attributes, "Expected tool.name attribute on call_tool span"
        assert tool_span.attributes["tool.name"] == "multiply"
        assert "tool.arguments" in tool_span.attributes, "Expected tool.arguments attribute on call_tool span"
...
async def test_agent_with_multiple_tools(instrument_legacy, span_exporter, log_exporter):
    ...
    task_span_names = {"CompactAndRefine.task", "TokenTextSplitter.task", "DefaultSQLParser.task"}
    assert len(task_span_names & span_names) > 0, "Expected at least one task span from the workflow"
    ...
    llm_spans_with_model = [
        span for span in cohere_spans
        if GenAIAttributes.GEN_AI_REQUEST_MODEL in span.attributes or "gen_ai.request.model" in span.attributes
    ]
    assert len(llm_spans_with_model) >= 1, "Expected at least one Cohere span with gen_ai.request.model"
    ...
    for cohere_span in llm_spans_with_model:
        ...
        assert GenAIAttributes.GEN_AI_REQUEST_MODEL in cohere_span.attributes or \
            "gen_ai.request.model" in cohere_span.attributes, \
            f"Expected gen_ai.request.model in {cohere_span.name}"

        # Check for prompt content attributes (gen_ai.prompt.X.content)
        prompt_keys = [k for k in cohere_span.attributes if k.startswith("gen_ai.prompt.")]
        ...
        completion_keys = [k for k in cohere_span.attributes if k.startswith("gen_ai.completion")]
        assert len(completion_keys) > 0, f"Expected completion attributes in {cohere_span.name}"

        # Check that llm.request.type exists
        assert SpanAttributes.LLM_REQUEST_TYPE in cohere_span.attributes or \
            "llm.request.type" in cohere_span.attributes, \
            f"Expected llm.request.type in {cohere_span.name}"

        # Check for token usage attributes (restored with Cohere format support)
        assert GenAIAttributes.GEN_AI_USAGE_OUTPUT_TOKENS in cohere_span.attributes, \
        ...
        assert GenAIAttributes.GEN_AI_USAGE_INPUT_TOKENS in cohere_span.attributes, \
        ...
        assert SpanAttributes.LLM_USAGE_TOTAL_TOKENS in cohere_span.attributes, \
    ...
    query_engine_tool_spans = [span for span in spans if span.name == "QueryEngineTool.task"]

    assert len(function_tool_spans) >= 1, "Expected at least one FunctionTool.task span (calc_tool)"
    assert len(query_engine_tool_spans) >= 1, "Expected at least one QueryEngineTool.task span (sql_tool)"

    # Check that task spans have traceloop.entity.name attribute
    for span in function_tool_spans + query_engine_tool_spans:
        assert "traceloop.entity.name" in span.attributes, \
            f"Expected traceloop.entity.name in {span.name}"

    # Verify tool.name and tool.arguments are set on call_tool spans
    call_tool_spans = [span for span in spans if span.name == "call_tool.task"]
    assert len(call_tool_spans) >= 2, "Expected at least 2 call_tool.task spans (sql_tool and calc_tool)"
    ...
    for tool_span in call_tool_spans:
        ...
        assert tool_name in ["sql_tool", "calc_tool"], f"Unexpected tool name: {tool_name}"
        assert "tool.arguments" in tool_span.attributes, "Expected tool.arguments attribute on call_tool span"
