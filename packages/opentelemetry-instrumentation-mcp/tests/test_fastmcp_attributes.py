...
async def test_fastmcp_comprehensive_attributes(span_exporter, tracer_provider) -> None:
    ...
    assert tool_span.attributes.get("traceloop.workflow.name") == "attribute-test-server"
...
async def test_fastmcp_error_handling(span_exporter, tracer_provider) -> None:
    ...
    if error_spans:
        ...
        assert error_span.attributes.get("traceloop.workflow.name") == "error-test-server"
