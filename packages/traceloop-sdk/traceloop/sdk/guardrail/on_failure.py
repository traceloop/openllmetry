...
def raise_exception(
    ...
    Example:
        on_failure=raise_exception("PII detected in response")
        on_failure=raise_exception("Invalid input", exception_type=ValueError)
    """
...
def log(
    ...
    Example:
        on_failure=log(message="Toxic content detected")
    """
...
def noop() -> Callable[["GuardedResult[Any, Any]"], Any]:
    ...
    Example:
        on_failure=noop()  # Just observe, don't block
    """
...
def return_value(
    ...
    Example:
        on_failure=return_value("Sorry, I can't help with that.")
        on_failure=return_value({"error": "blocked", "reason": "PII"})
    """
