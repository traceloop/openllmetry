...
def test_generate(exporter_legacy, watson_ai_model, log_exporter):
    ...
    assert (
        watsonx_ai_span.attributes[f"{SpanAttributes.GEN_AI_PROMPT}.0.user"]
        == "What is 1 + 1?"
    )
    assert watsonx_ai_span.attributes[SpanAttributes.GEN_AI_SYSTEM] == "Watsonx"
    assert watsonx_ai_span.attributes.get(f"{SpanAttributes.GEN_AI_COMPLETION}.0.content")
...
def test_generate_text_stream(exporter_legacy, watson_ai_model, log_exporter):
    ...
    assert watsonx_ai_span.attributes[SpanAttributes.GEN_AI_SYSTEM] == "Watsonx"
    assert (
        watsonx_ai_span.attributes[f"{SpanAttributes.GEN_AI_COMPLETION}.0.content"]
        == generated_text
    )
    assert watsonx_ai_span.attributes.get(SpanAttributes.LLM_USAGE_TOTAL_TOKENS)
