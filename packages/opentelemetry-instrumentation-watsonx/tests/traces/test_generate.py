...
def test_generate(exporter, watson_ai_model):
    ...
    assert watsonx_ai_span.attributes[f"{SpanAttributes.LLM_PROMPTS}.0.user"] == "What is 1 + 1?"
...
def test_generate_text_stream(exporter, watson_ai_model):
    ...
    spans = exporter.get_finished_spans()
    watsonx_ai_span = spans[1]
    assert watsonx_ai_span.attributes[f"{SpanAttributes.LLM_PROMPTS}.0.user"] == "Write an epigram about the sun"
    assert watsonx_ai_span.attributes[SpanAttributes.LLM_SYSTEM] == "Watsonx"
    assert watsonx_ai_span.attributes[f"{SpanAttributes.LLM_COMPLETIONS}.0.content"] == generated_text
    assert watsonx_ai_span.attributes.get(SpanAttributes.LLM_USAGE_TOTAL_TOKENS)
