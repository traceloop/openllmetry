...
async def test_apply_prompt_safety_via_to_thread():
    ...
    kwargs = {"json": {"messages": [{"role": "user", "content": "hello"}]}}
    result = await asyncio.to_thread(
        _apply_prompt_safety, None, kwargs, LLMRequestTypeValues.CHAT, "ollama.chat"
    )
...
async def test_apply_completion_safety_via_to_thread():
    ...
    await asyncio.to_thread(
        _apply_completion_safety, None, response, LLMRequestTypeValues.CHAT, "ollama.chat"
    )
