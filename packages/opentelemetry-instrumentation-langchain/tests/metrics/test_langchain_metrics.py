...
def test_langgraph_metrics(instrument_legacy, reader, openai_client):
    ...
    token_usage_metric = next(
        (m for m in metric_data if m.name == Meters.LLM_TOKEN_USAGE),
        None,
    )
    ...
    assert token_usage_data_point.attributes[
        GenAIAttributes.GEN_AI_PROVIDER_NAME
    ] == "openai" and token_usage_data_point.attributes[GenAIAttributes.GEN_AI_TOKEN_TYPE] in ["input", "output"]

    duration_metric = next(
        (m for m in metric_data if m.name == Meters.LLM_OPERATION_DURATION),
        None,
    )
    ...
    generation_choices_metric = next((m for m in metric_data if m.name == Meters.LLM_GENERATION_CHOICES), None)
