...
def test_batch_metadata_in_span_attributes(instrument_legacy, span_exporter):
    ...
    for span in llm_spans:
        ...
        assert "user_id" in span.attributes or f"{SpanAttributes.TRACELOOP_ASSOCIATION_PROPERTIES}.user_id" in span.attributes
        assert "session_id" in span.attributes or f"{SpanAttributes.TRACELOOP_ASSOCIATION_PROPERTIES}.session_id" in span.attributes
        assert "priority" in span.attributes or f"{SpanAttributes.TRACELOOP_ASSOCIATION_PROPERTIES}.priority" in span.attributes
        
        # Check the values
        user_id_attr = span.attributes.get("user_id") or span.attributes.get(f"{SpanAttributes.TRACELOOP_ASSOCIATION_PROPERTIES}.user_id")
        session_id_attr = span.attributes.get("session_id") or span.attributes.get(f"{SpanAttributes.TRACELOOP_ASSOCIATION_PROPERTIES}.session_id")
        priority_attr = span.attributes.get("priority") or span.attributes.get(f"{SpanAttributes.TRACELOOP_ASSOCIATION_PROPERTIES}.priority")
...
async def test_async_batch_metadata_in_span_attributes(instrument_legacy, span_exporter):
    ...
    for span in llm_spans:
        ...
        assert "user_id" in span.attributes or f"{SpanAttributes.TRACELOOP_ASSOCIATION_PROPERTIES}.user_id" in span.attributes
        assert "session_id" in span.attributes or f"{SpanAttributes.TRACELOOP_ASSOCIATION_PROPERTIES}.session_id" in span.attributes
        assert "environment" in span.attributes or f"{SpanAttributes.TRACELOOP_ASSOCIATION_PROPERTIES}.environment" in span.attributes
        
        # Check the values
        user_id_attr = span.attributes.get("user_id") or span.attributes.get(f"{SpanAttributes.TRACELOOP_ASSOCIATION_PROPERTIES}.user_id")
        session_id_attr = span.attributes.get("session_id") or span.attributes.get(f"{SpanAttributes.TRACELOOP_ASSOCIATION_PROPERTIES}.session_id")
        environment_attr = span.attributes.get("environment") or span.attributes.get(f"{SpanAttributes.TRACELOOP_ASSOCIATION_PROPERTIES}.environment")
