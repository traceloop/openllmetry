...
def test_nova_completion(test_context, brt):
    ...
    # Assert on prompt
    assert (
            bedrock_span.attributes[f"{SpanAttributes.LLM_PROMPTS}.1.role"]
            == "user"
    )
...
def test_nova_invoke_stream(test_context, brt):
    ...
    # Assert on other request parameters
    assert bedrock_span.attributes[SpanAttributes.LLM_REQUEST_MAX_TOKENS] == inf_params.get("maxTokens")
    assert bedrock_span.attributes[SpanAttributes.LLM_REQUEST_TEMPERATURE] == inf_params.get("temperature")
    assert bedrock_span.attributes[SpanAttributes.LLM_REQUEST_TOP_P] == inf_params.get("topP")
    # There is no response id for Amazon Titan models in the response body,
    # only request id in the response.
    assert bedrock_span.attributes.get("gen_ai.response.id") is None
