...
def test_nova_completion(instrument_legacy, brt, span_exporter, log_exporter):
    ...
    assert bedrock_span.attributes[SpanAttributes.GEN_AI_REQUEST_MODEL] == "nova-lite-v1:0"
    ...
    assert bedrock_span.attributes[
        f"{SpanAttributes.GEN_AI_PROMPT}.1.content"
    ...
    for i in range(0, len(generated_text)):
        assert (
            bedrock_span.attributes[f"{SpanAttributes.GEN_AI_COMPLETION}.{i}.content"]
            == generated_text[i]["text"]
        )

    # Assert on other request parameters
    assert bedrock_span.attributes[SpanAttributes.GEN_AI_REQUEST_MAX_TOKENS] == 500
    assert bedrock_span.attributes[SpanAttributes.GEN_AI_REQUEST_TEMPERATURE] == 0.7
    assert bedrock_span.attributes[SpanAttributes.GEN_AI_REQUEST_TOP_P] == 0.9
...
def test_nova_invoke_stream(instrument_legacy, brt, span_exporter, log_exporter):
    ...
    assert bedrock_span.attributes[SpanAttributes.GEN_AI_REQUEST_TOP_P] == inf_params.get(
        "topP"
    )
    # There is no response id for Amazon Titan models in the response body,
    # only request id in the response.
...
def test_nova_converse(instrument_legacy, brt, span_exporter, log_exporter):
    ...
    # Assert on system prompt
    assert bedrock_span.attributes[f"{SpanAttributes.GEN_AI_PROMPT}.0.role"] == "system"
    assert bedrock_span.attributes[f"{SpanAttributes.GEN_AI_PROMPT}.0.content"] == system[
        0
    ].get("text")
    ...
    # Assert on other request parameters
    assert bedrock_span.attributes[SpanAttributes.GEN_AI_REQUEST_MAX_TOKENS] == 300
    assert bedrock_span.attributes[SpanAttributes.GEN_AI_REQUEST_TEMPERATURE] == 0.3
    assert bedrock_span.attributes[SpanAttributes.GEN_AI_REQUEST_TOP_P] == 0.1
...
def test_nova_converse_stream(instrument_legacy, brt, span_exporter, log_exporter):
    ...
    assert (
        bedrock_span.attributes[SpanAttributes.GEN_AI_USAGE_INPUT_TOKENS] == inputTokens
    )
    assert (
...
def test_nova_cross_region_invoke(instrument_legacy, brt, span_exporter, log_exporter):
    ...
    # Assert on prompt
    assert bedrock_span.attributes[f"{SpanAttributes.GEN_AI_PROMPT}.0.role"] == "user"
