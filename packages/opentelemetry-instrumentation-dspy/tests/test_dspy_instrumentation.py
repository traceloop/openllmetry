...
from opentelemetry.instrumentation.dspy.instrumentation import (
    _infer_provider,
)
from opentelemetry.instrumentation.dspy.utils import response_to_otel_output


def test_response_to_otel_output_handles_legacy_string_list():
    payload = json.loads(response_to_otel_output(["answer"]))

    assert payload == [{"role": "assistant", "parts": [{"type": "text", "content": "answer"}]}]


def _fake_litellm_result(
    model="openai/gpt-4o",
    prompt_tokens=10,
    completion_tokens=5,
    cache_hit=False,
...
def test_lm_forward_omits_content_when_tracing_is_disabled(monkeypatch, span_exporter):
    monkeypatch.setenv("TRACELOOP_TRACE_CONTENT", "false")
    lm = dspy.LM("openai/gpt-4o", cache=False)
    fake = _fake_litellm_result()

    with patch("dspy.clients.lm.litellm_completion", return_value=fake):
        lm.forward(messages=[{"role": "user", "content": "secret"}])
...
def test_lm_forward_positional_prompt(span_exporter):
    lm = dspy.LM("openai/gpt-4o", cache=False)
    fake = _fake_litellm_result()
    ...
    assert payload[0]["parts"][0]["content"] == "hello"
...
@pytest.mark.parametrize("model", ["o1abc", "o3third-party", "o4custom"])
def test_provider_inference_does_not_overmatch_short_openai_prefixes(model):
    assert _infer_provider(model) is None


@pytest.mark.parametrize("model", ["o1", "o1-preview", "o3", "o3-mini", "o4", "o4-mini"])
def test_provider_inference_accepts_openai_reasoning_model_ids(model):
    assert _infer_provider(model) == GenAISystem.OPENAI.value


def test_lm_wrappers_annotate_disabled_metrics_as_optional():
    module = inspect.getmodule(_infer_provider)
    functions = {
        node.name: node
        for node in ast.walk(ast.parse(inspect.getsource(module)))
        if isinstance(node, (ast.FunctionDef, ast.AsyncFunctionDef))
    }
    for wrapper_name in ("wrap_lm_forward", "wrap_lm_aforward"):
        parameters = functions[wrapper_name].args.args
        assert ast.unparse(parameters[1].annotation) == "Histogram | None"
        assert ast.unparse(parameters[2].annotation) == "Histogram | None"
...
@pytest.mark.asyncio
async def test_lm_aforward_omits_content_when_tracing_is_disabled(monkeypatch, span_exporter):
    monkeypatch.setenv("TRACELOOP_TRACE_CONTENT", "false")
    lm = dspy.LM("openai/gpt-4o", cache=False)
    fake = _fake_litellm_result()
    ...
    with patch("dspy.clients.lm.alitellm_completion", side_effect=fake_acompletion):
        await lm.aforward(messages=[{"role": "user", "content": "secret"}])
