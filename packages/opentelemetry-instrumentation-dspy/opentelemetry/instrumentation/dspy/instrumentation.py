...
def wrap_lm_forward(
    ...
    with tracer.start_as_current_span(
        ...
        _safe_set_lm_span_input(span, args, kwargs)
        ...
        try:
            ...
            _safe_record_lm_duration(duration_histogram, result, model, provider, start)
        _safe_set_lm_span_output(span, result, model, provider, token_histogram)
...
def _safe_set_lm_span_input(span, args, kwargs):
    try:
        if should_send_prompts():
            input_json = messages_to_otel_input(_extract_messages(args, kwargs))
            if input_json:
                set_span_attribute(span, GenAIAttributes.GEN_AI_INPUT_MESSAGES, input_json)
    except Exception:
        # Optional telemetry (including logging handlers) must not block the call.
        pass


def _safe_record_lm_duration(duration_histogram, result, model, provider, start):
    ...
    try:
        ...
        attrs = {}
        if response_model:
            attrs[GenAIAttributes.GEN_AI_RESPONSE_MODEL] = str(response_model)
        if provider:
            attrs[GenAIAttributes.GEN_AI_PROVIDER_NAME] = provider
        duration_histogram.record(time.time() - start, attributes=attrs)
    except Exception:
        # Never replace a result, application error, or cancellation in finally.
        pass


def _safe_set_lm_span_output(span, result, model, provider, token_histogram):
    """Telemetry must never break the user's call; swallow post-call errors."""
    try:
        _set_lm_span_output(span, result, model, provider, token_histogram)
    except Exception:
        try:
            logger.debug("Failed to set LM span output", exc_info=True)
        except Exception:
            # A user logging handler must not replace the application result.
            pass
...
def _set_lm_span_output(span, result, model, provider, token_histogram):
    if should_send_prompts():
