...
def test_prompt_safety_masks_chat_message():
    ...
    with tracer.start_as_current_span("together.chat") as span:
        updated_kwargs = _apply_prompt_safety(
            span, kwargs, LLMRequestTypeValues.CHAT, "together.chat"
        )

    assert updated_kwargs["messages"][0]["content"] == "[PII.chat]"


def test_completion_safety_masks_completion_choice():
    ...
    with tracer.start_as_current_span("together.completion") as span:
        _apply_completion_safety(
            span, response, LLMRequestTypeValues.COMPLETION, "together.completion"
        )
...
def test_prompt_and_completion_cover_prompt_and_chat_branches():
    _, tracer = _test_span()
    register_prompt_safety_handler(
        lambda context: SafetyResult(
    ...
    with tracer.start_as_current_span("together.chat") as span:
        ...
        _apply_completion_safety(span, response, LLMRequestTypeValues.CHAT, "together.chat")
        unchanged = _apply_prompt_safety(
            span, {"messages": "invalid"}, LLMRequestTypeValues.CHAT, "together.chat"
        )
